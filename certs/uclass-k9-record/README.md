# the distinct-letter window at nine orbits — producing-run record

Record of the orbit-bounded walk behind the paper's distinct-letter window theorem, at nine vertex
orbits over 1–54 chambers, in the valence-2 regime. The walk takes two species-level prunes
...
A *record*, separate from `certs/uclass-k9`, where a fresh run writes its own outputs.
