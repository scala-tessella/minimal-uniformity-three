...
```bash
sbt test    # every claim; exact, in-JVM, no external tools (research-core 0.3.0 from Maven Central)
```
...
| The k ≤ 2 catalogue & scan | `UClassK2Probe` | Enumerates the ≤ 2-orbit catalogue and scans it for class-U(z) survivors. |
...
| The k = 3 all-species search | `UClassK3Probe` | The exhaustive k = 3 search across all species. |
